...
param ([String] $if_name, [String] $ip, [String] $mask, [String] $gw)
    Write-Verbose "Set-FixedIP -IFName $IFName -IP $IP -Mask $Mask -Gateway $Gateway"
	if ($IFName -ne $false){
        if ( $false -eq $IP -or $false -eq $Mask ){
            Throw "Invalid Set-FixedIP call missing parameters"
        }
        if ($gw -ne $false){
		    Write-Verbose "netsh.exe interface ipv4 set address name=$IFName static $IP $Mask $Gateway" 
		    netsh.exe interface ipv4 set address name=$IFName static $IP $Mask $Gateway 
	    } else {
		Write-Verbose "netsh.exe interface ipv4 set address name=$IFName static $IP $Mask"
